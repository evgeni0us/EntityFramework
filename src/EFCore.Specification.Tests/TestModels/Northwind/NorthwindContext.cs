...
namespace Microsoft.EntityFrameworkCore.Specification.Tests.TestModels.Northwind
{
    public class NorthwindContext : DbContext
    {
        ...
        public void ConfigureFilters(ModelBuilder modelBuilder)
        {
            ...
            modelBuilder.Entity<Customer>().HasFilter(c => c.CompanyName.StartsWith(TenantPrefix));
            modelBuilder.Entity<OrderDetail>().HasFilter(od => od.Quantity > 50);
            modelBuilder.Entity<Employee>().HasFilter(e => e.Address.StartsWith("A"));
            modelBuilder.Entity<Product>().HasFilter(p => ClientMethod(p));
        }
